...
int compareByWeightAndName(const Pokemon *p1, const Pokemon *p2) {
    if (p1->weight < p2->weight) return -1;
    if (p1->weight > p2->weight) return 1;
    return strcmp(p1->name, p2->name);
}

void shellSort(Pokemon *pokedex, int n) {
    for (int gap = n / 2; gap > 0; gap /= 2) {
        for (int i = gap; i < n; i++) {
            Pokemon temp = pokedex[i];
            int j;
            for (j = i; j >= gap && compareByWeightAndName(&temp, &pokedex[j - gap]) < 0; j -= gap) {
...
int main () 
{
    ...
    FILE *arquivo = fopen("848324_shellsort.txt", "w");
    if (arquivo == NULL) {
