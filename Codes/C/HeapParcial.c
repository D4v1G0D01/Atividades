...
// Declarações (prototipação) das funções
...
// Função para comparar por altura e, em caso de empate, por nome
int compareByHeightAndName(const Pokemon *p1, const Pokemon *p2) {
    if (p1->height < p2->height) return -1;
    if (p1->height > p2->height) return 1;
    return strcmp(p1->name, p2->name);
}

// Função auxiliar para "heapify"
void heapify(Pokemon *pokedex, int n, int i) {
    int largest = i;  
    int left = 2 * i + 1;
    int right = 2 * i + 2;

    if (left < n && compareByHeightAndName(&pokedex[left], &pokedex[largest]) > 0)
        largest = left;

    if (right < n && compareByHeightAndName(&pokedex[right], &pokedex[largest]) > 0)
        largest = right;

    if (largest != i) {
        Pokemon temp = pokedex[i];
        pokedex[i] = pokedex[largest];
        pokedex[largest] = temp;
...
void heapsort(Pokemon *pokedex, int n) {
    for (int i = n / 2 - 1; i >= 0; i--)
        heapify(pokedex, n, i);

    for (int i = n - 1; i >= 0; i--) {
        Pokemon temp = pokedex[0];
        pokedex[0] = pokedex[i];
        pokedex[i] = temp;
...
int main () {
    ...
    FILE *arquivo = fopen("848324_heapsort.txt", "w");
    if (arquivo == NULL) {
